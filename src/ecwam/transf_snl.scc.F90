...
!
MODULE TRANSF_SNL_SCC_MOD
  CONTAINS
  FUNCTION TRANSF_SNL_SCC (XK0, D, XNU, SIG_TH)
...
!$acc routine seq
    ...
    IF (D < BATHYMAX .and. D > 0._JWRB) THEN
      ...
      ELSE
        ...
        TRANSF_SNL_SCC = XNL**2 / (DV_G*T_0_SQ**4)
        TRANSF_SNL_SCC = MAX(MIN(TRANSF_SNL_MAX, TRANSF_SNL_SCC), TRANSF_SNL_MIN)
      END IF
    ELSE
      TRANSF_SNL_SCC = 1._JWRB
    END IF
  ...
  END FUNCTION TRANSF_SNL_SCC
END MODULE TRANSF_SNL_SCC_MOD

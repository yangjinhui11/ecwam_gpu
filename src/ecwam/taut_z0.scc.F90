...
!
MODULE TAUT_Z0_SCC_MOD
  CONTAINS
  SUBROUTINE TAUT_Z0_SCC (KIJS, KIJL, IUSFG, HALP, UTOP, UDIR, TAUW, TAUWDIR, RNFAC, USTAR, Z0, Z0B, CHRNCK)
    ...
    USE STRESS_GC_SCC_MOD, ONLY: STRESS_GC_SCC
    ...
    USE YOWPHYS, ONLY: XKAPPA, XNLEV, RNU, RNUM, ALPHA, ALPHAMIN, ALPHAMAX, ANG_GC_A, ANG_GC_B, ANG_GC_C
    ...
    LOGICAL :: LLCOSDIFF
!$acc routine vector
!$acc data present( HALP, UTOP, UDIR, TAUW, TAUWDIR, RNFAC, USTAR, Z0, Z0B, CHRNCK )
...
!$acc loop vector
    DO IJ=KIJS,KIJL
      ...
      IF (LLGCBZ0) THEN
        ...
        IF (LLCAPCHNK) THEN
          CHARNOCK_MIN = CHNKMIN_SCC(UTOP(IJ))
...
!$acc loop seq
        DO ITER=1,NITER
          ...
          TAUUNR = STRESS_GC_SCC(ANG_GC, USTAR(IJ), Z0(IJ), Z0MIN, HALP(IJ), RNFAC(IJ))
...
!$acc loop seq
          DO ITER=1,NITER
            ...
            TAUUNR = STRESS_GC_SCC(ANG_GC, USTOLD, Z0(IJ), Z0MIN, HALP(IJ), RNFAC(IJ))
...
!$loki routine seq
  ...
  END SUBROUTINE TAUT_Z0_SCC
END MODULE TAUT_Z0_SCC_MOD

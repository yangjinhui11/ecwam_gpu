...
CONTAINS
SUBROUTINE PROPAGS1_SCC (F1, F3, NINF, NSUP, KIJS, KIJL,           &
 &                   BLK2GLO,                                  &
 &                   DEPTH_EXT,                                &
...
!Yangjinhui 
    USE CHECKCFL_MOD , ONLY: CHECKCFL_SCC
! ----------------------------------------------------------------------
...
!$acc loop vector private(DRDP,DRDM,DLA,DTC,DPH)
      DO K=1,NANG
        SD = SINTH(K)
        CD = COSTH(K)
...
!             the non rotated quadrant has the strictest CFL criteria
              ...
              ENDDO
              CALL CHECKCFL_SCC(KIJS, KIJL, DEPTH_EXT(KIJS), DTC,     &

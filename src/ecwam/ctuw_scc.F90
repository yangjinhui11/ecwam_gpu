...
CONTAINS
SUBROUTINE CTUW_SCC (DELPRO, MSTART, MEND,                    & 
...
!             ---------------------
!$acc loop vector private(ADXP,ADYP,DXUP,DYUP,DXDW,DYDW,WEIGHT)
              DO IJ=KIJS,KIJL
                IX=BLK2GLO%IXLG(IJ)
                KY=BLK2GLO%KXLT(IJ)
...
!$acc loop vector private(DRGP,DRGM,DRDP,DRDM,DRCP,DRCM)
      DO K=1,NANG
        KP1 = K+1
        IF (KP1 > NANG) KP1 = 1
        KM1 = K-1
